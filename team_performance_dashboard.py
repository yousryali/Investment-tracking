...
# Constants
DEFAULT_EXCHANGE_API = "https://api.exchangerate.host/latest?base=USD&symbols=EGP"
...
try:
    response = requests.get(DEFAULT_EXCHANGE_API)
    exchange_rate = response.json()['rates']['EGP']
except:
    exchange_rate = 50.0  # fallback
...
def to_display_currency(usd=None, egp=None):
    if currency_display == "USD":
        return usd + (egp / exchange_rate if egp else 0)
    else:
        return egp + (usd * exchange_rate if usd else 0)
